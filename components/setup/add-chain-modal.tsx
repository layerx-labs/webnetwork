...
export default function AddChainModal({
  ...
  const [privateRpc, setPrivateRpc] = useState(chain?.rpc?.[0]);
  ...
  async function handleAddChain() {
    ...
    add({...chain, activeRPC, privateRpc, eventsApi, explorer, color, icon: ipfsHash })
  }

  return (
    <Modal show={show} okLabel={'add chain'}
           title="Configure RPC"
           onOkClick={handleAddChain}
           okDisabled={!validUrl(activeRPC) || !validUrl(privateRpc) || !validUrl(eventsApi) || !validUrl(explorer)}
      ...
      <Row>
        <ReactSelect defaultValue={{value: chain.rpc[0], label: chain.rpc[0]}}
                     ...
                     onChange={e => {
                       setActiveRPC(e.value);
                       setPrivateRpc(e.value);
                     }}></ReactSelect>
      </Row>
      <Row className="mt-3 mb-2">
        <Col>Configure Public RPC</Col>
      </Row>
      <Row>
        <Col>
          <FormControl
