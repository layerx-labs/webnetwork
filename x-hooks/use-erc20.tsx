...
export default function useERC20() {
  ...
  async function updateAllowanceAndBalance() {
    ...
    const balance = await state.Service?.active.getTokenBalance(address, state.currentUser.walletAddress)
      .then(b => {
        setBalance(b);
        return b;
      })
    ...
    if (realSpender)
      allowance = await state.Service?.active.getAllowance(address, state.currentUser.walletAddress, realSpender)
        .then(a => {
          setAllowance(a);
          return a;
        })
